...
import (
	...
	"github.com/coramtix-dev/wings/server/backup"
)
...
// Get all of the ignored files for a server based on its .coramtixignore file in the root.
func (s *Server) getServerwideIgnoredFiles() (string, error) {
	f, st, err := s.Filesystem().File(".coramtixignore")
	if err != nil {
