...
import (
	...
	"github.com/coramtix-dev/wings/system"
)

const DefaultLocation = "/etc/coramtix/config.yml"
...
type SystemConfiguration struct {
	// The root directory where all of the coramtix data is stored at.
	RootDirectory string `default:"/var/lib/coramtix" json:"-" yaml:"root_directory"`

	// Directory where logs for server installations and other wings events are logged.
	LogDirectory string `default:"/var/log/coramtix" json:"-" yaml:"log_directory"`

	// Directory where the server data is stored at.
	Data string `default:"/var/lib/coramtix/volumes" json:"-" yaml:"data"`

	// Directory where server archives for transferring will be stored.
	ArchiveDirectory string `default:"/var/lib/coramtix/archives" json:"-" yaml:"archive_directory"`

	// Directory where local backups will be stored on the machine.
	BackupDirectory string `default:"/var/lib/coramtix/backups" json:"-" yaml:"backup_directory"`

	// TmpDirectory specifies where temporary files for coramtix installation processes
	// should be created. This supports environments running docker-in-docker.
	TmpDirectory string `default:"/tmp/coramtix" json:"-" yaml:"tmp_directory"`

	// The user that should own all of the server files, and be used for containers.
	Username string `default:"coramtix" yaml:"username"`
	...
	User struct {
		...
		PasswdFile string `json:"passwd_file" yaml:"passwd_file" default:"/etc/coramtix/passwd"`
	} `yaml:"user"`
...
type Configuration struct {
	...
	AppName string `default:"coramtix" json:"app_name" yaml:"app_name"`
...
// EnsureCoRamTixUser ensures that the CoRamTix core user exists on the
...
func EnsureCoRamTixUser() error {
	...
	if sysName == "distroless" {
		_config.System.Username = system.FirstNotEmpty(os.Getenv("WINGS_USERNAME"), "coramtix")
	...
	log.WithField("username", _config.System.Username).Info("checking for coramtix system user")
