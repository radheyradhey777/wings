...
## v1.11.11
### Fixed
* Backups missing content when a `.coramtixignore` file is used
...
## v1.11.5
### Added
* Added a config option to disable Wings config.yml updates from the Panel (https://github.com/coramtix-dev/wings/commit/ec6d6d83ea3eb14995c24f001233e85b37ffb87b)

### Changed
* Wings is now built with Go 1.19.7

### Fixed
* Fixed archives containing partially matched file names (https://github.com/coramtix-dev/wings/commit/43b3496f0001cec231c80af1f9a9b3417d04e8d4)
...
* Adds support for configuring the MTU on the `coramtix0` network.
...
### Added
* Added the ability to define the location of the temporary folder used by Wings — defaults to `/tmp/coramtix`.
...
* Fixes files uploaded with SFTP not being owned by the CoRamTix user.
...
* Fixes port bindings being incorrectly re-assigned when using `127.0.0.1` with Docker. These are now properly re-mapped to the `coramtix0` interface so that networking operates as expected for the server.
...
## v1.0.0-beta.5
### Fixed
* Default config location settled on `/etc/coramtix/config.yml`; wings will now check all of the previous locations for the configuration and move it automatically to the new location.
